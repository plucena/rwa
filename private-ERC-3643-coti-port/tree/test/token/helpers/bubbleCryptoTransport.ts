...
import { Wallet, HDNodeWallet, utils, BigNumber } from "ethers";

/** ethers v5: `getBytes` / `solidityPacked` live on `utils` (v6 moved them to top-level). */
function getBytes(data: string | Uint8Array): Uint8Array {
  return new Uint8Array(utils.arrayify(data));
}

function solidityPacked(types: string[], values: unknown[]): string {
  return utils.solidityPack(types, values as any[]);
}

function makeNoKeepAliveHttpsAgent() {
  // Prevent lingering sockets/open handles in Node test runners.
...
function handleToBytes(handle: bigint): Uint8Array {
  return getBytes(handleToHex(handle));
}
...
export async function getUserKeyViaProxy(signer: Wallet | HDNodeWallet, proxyUrl: string) {
  ...
  const addressBytes = getBytes(userAddress);
  ...
  const shouldRetryOnboardStatus = (status: number): boolean => {
    return status === 408 || status === 429 || status >= 500;
  };

  const isTransientOnboardError = (err: unknown): boolean => {
  ...
  const result = await retryWithBackoff(async () => {
    ...
    let response;
    try {
      response = await fetch(`${normalizedProxyUrl}/onboard`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(requestData),
        agent: makeNoKeepAliveHttpsAgent(),
      });
    } catch (error) {
      if (isTransientOnboardError(error)) {
        throw error;
      }
      throw new Error(`Onboarding failed: ${String((error as { message?: string })?.message || error)}`);
    }
    ...
    if (!response.ok) {
      ...
      if (shouldRetryOnboardStatus(response.status)) {
        throw new Error(`Transient onboarding error HTTP ${response.status}: ${errorText}`);
      }
      throw new Error(`Onboarding failed: ${errorText}`);
    }

    return (await response.json()) as { rsa_ciphertexts: string; message: string };
  }, 4, 1500);
...
export async function decryptValueViaProxy(
  ...
  const userSignature = base64EncodeBytes(getBytes(signature));
...
export function prepareMessageForBubble128(
  ...
  if (typeof contractAddress !== "string" || !utils.isAddress(contractAddress)) {
  ...
  const messageBytes = solidityPacked(
    ...
    [signerAddress, contractAddress, BigNumber.from(encryptedInt.toString())]
  );
...
export function prepareMessageForBubble256(
  ...
  const messageBytes = solidityPacked(
    ...
    [
      signerAddress,
      contractAddress,
      BigNumber.from(encryptedHigh.toString()),
      BigNumber.from(encryptedLow.toString()),
    ]
  );
