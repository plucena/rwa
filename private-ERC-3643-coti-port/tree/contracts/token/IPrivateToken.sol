...
interface IPrivateToken {
    ...
    // B2: a COTI ciphertext has one reader, so each party gets its own copy.
     ...
     *  @param _it Ciphertext payload for amount of tokens to freeze
     *  This function can only be called by a wallet set as agent of the token
     *  emits a `TokensFrozen` event
     */
    function freezePartialTokens(address _userAddress, itUint256 calldata _it) external;

    /**
     *  @dev unfreezes token amount specified for given address
     *  @param _userAddress The address for which to update frozen tokens
     *  @param _it Ciphertext payload for amount of tokens to unfreeze
     *  This function can only be called by a wallet set as agent of the token
     *  emits a `TokensUnfrozen` event
     */
    function unfreezePartialTokens(address _userAddress, itUint256 calldata _it) external;
     ...
     *  @param _its Ciphertext payloads for transfer amounts to corresponding receivers
     *  emits _toList.length `Transfer` events
     */
    function batchTransfer(address[] calldata _toList, itUint256[] calldata _its) external;
     ...
     *  USE WITH CARE OR YOU COULD LOSE TX FEES WITH AN "OUT OF GAS" TRANSACTION
     *  @param _toList The addresses of the receivers
     *  @param _amounts Clear amounts of tokens to mint to the corresponding receiver (same semantics as `mint`)
     *  This function can only be called by a wallet set as agent of the token
     *  emits _toList.length `Transfer` events
     */
     ...
     *  @notice Hard cap on row count (`PrivateToken.MAX_BATCH_BURN_SIZE`). See `docs/BatchBurnOperations.md` for
     *  requested vs effective burn, multi-row semantics, and stuck decrypt handling.
     *  @param _userAddresses The addresses of the wallets concerned by the burn
     *  @param _amounts Clear amounts to burn from the corresponding wallets (same semantics as `burn`)
     *  This function can only be called by a wallet set as agent of the token
     *  emits _userAddresses.length `BurnRequested` events (one decrypt id for the batch)
     */
    function batchBurn(address[] calldata _userAddresses, uint256[] calldata _amounts) external;

    /// @return True if a `callbackBurn` is still expected for this id (single or batch metadata present).
    function isBurnDecryptPending(uint256 decryptID) external view returns (bool);

    /// @return Users registered for a pending batch burn at `decryptID`, or empty if none / not a batch row.
    function getPendingBatchBurnUsers(uint256 decryptID) external view returns (address[] memory);

    /// @return User registered for a pending single burn at `decryptID`, or zero if none.
     ...
     */
    function batchFreezePartialTokens(address[] calldata _userAddresses, gtUint256[] calldata _amounts) external;
     ...
     */
    function batchUnfreezePartialTokens(address[] calldata _userAddresses, gtUint256[] calldata _amounts) external;
