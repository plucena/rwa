...
interface IPrivateModularCompliance {
    ...
    /**
     *  @dev Returns an encrypted transfer-policy allow bit.
     *  @param _from The address of the sender
     *  @param _to The address of the receiver
     *  @param _amount The amount of tokens involved in the transfer
     *  This function may call module checks and aggregate them into a single encrypted
     *  policy result consumed by `PrivateToken` (legacy name: `PravateToken`).
     *  Return semantics are intentionally "allow-oriented":
     *  - `gtBool(true)`  => transfer/mint is allowed by compliance
     *  - `gtBool(false)` => transfer/mint must be blocked
     *
     *  Not `view`: private / MPC compliance may invoke on-chain handlers (e.g. SetPublic),
     *  which must not be executed via STATICCALL.
     */
